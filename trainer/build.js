...
function main() {
  ...
  let dictionary = [];
  raw.split("\n").map((word, index) => {
    if (application.filter(word) && index % 10 === 0) {
      dictionary.push(application.format(word));
    }
  });
  ...
  for (let file of fs.readdirSync(`${root}/data/cases/train`).sort()) {
    ...
    for (let string in fixture) {
      if (fixture.hasOwnProperty(string) && fixture[string] === false && application.filter(string)) {
